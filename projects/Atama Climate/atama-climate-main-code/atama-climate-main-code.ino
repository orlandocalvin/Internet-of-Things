...
// ===== LOGGING COUNTERS =====
volatile uint32_t readingCounter = 0;
volatile uint32_t currentPostingReading = 0;
...
void checkPostStatus() {
  ...
  if (response.indexOf("+HTTPACTION:") != -1) {
    if (response.indexOf(",200,") != -1) {
      Serial.println("Status: POST SUCCESS");
    } else {
...
void sendSensorData() {
  ...
  serializeJsonPretty(data, log);
  
  // Increment reading number and snapshot for this POST
  readingCounter++;
  currentPostingReading = readingCounter;

  // ===== SERIAL LOGGING =====
  Serial.println();
  Serial.println("===== READING #" + String(currentPostingReading) + " =====");
  Serial.println("Timestamp: " + String(timeStamp));
  Serial.println("Payload: " + log);
